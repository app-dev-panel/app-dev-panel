...
### [ ] B1 — Kernel PHP version consistency
...
### [ ] B2 — Kernel Storage docs completeness
...
### [ ] B3 — API CLAUDE.md: HttpMock section missing
...
### [ ] B4 — Laravel adapter: missing listeners
...
### [ ] B5 — Strip "Overview" filler heading
...
### [ ] E2 — Endpoints table vs ApiRoutes.php audit
- `libs/API/CLAUDE.md` endpoint tables vs `libs/API/src/ApiRoutes.php` — 46 `Route(...)` registrations, docs have 80+ rows.
- Fix: script a diff (or grep both sides), add missing endpoints for Llm, Mcp, Services, HttpMock, remove any rows that no longer correspond to a route.
