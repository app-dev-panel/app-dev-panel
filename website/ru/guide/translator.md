---
title: Переводчик
description: "ADP автоматически перехватывает вызовы переводчика и записывает каждый запрос, включая отсутствующие переводы."
---
...
<class>AppDevPanel\Kernel\Collector\TranslatorCollector</class> реализует <class>AppDevPanel\Kernel\Collector\SummaryCollectorInterface</class> и фиксирует каждый запрос перевода во время выполнения запроса.

### TranslationRecord

Каждый вызов перевода создаёт DTO <class>AppDevPanel\Kernel\Collector\TranslationRecord</class>:
...
Каждый адаптер предоставляет proxy переводчика, который оборачивает нативный переводчик фреймворка и передаёт данные в <class>AppDevPanel\Kernel\Collector\TranslatorCollector</class>. Proxy регистрируются автоматически — ручная настройка не требуется.
...
**Подключение:** Регистрируется через <class>AppDevPanel\Adapter\Symfony\DependencyInjection\CollectorProxyCompilerPass</class> с помощью паттерна `setDecoratedService()` Symfony.
...
Перехват переводов включён по умолчанию, когда активен <class>AppDevPanel\Kernel\Collector\TranslatorCollector</class>. Дополнительная настройка не требуется.
