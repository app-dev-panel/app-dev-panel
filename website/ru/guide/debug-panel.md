...
::: tip Автоопределение
Когда `static_url` пустой (по умолчанию), каждый адаптер ищет бандлы в таком порядке:

1. **`app-dev-panel/frontend-assets`** — канонический Composer-пакет, ставится транзитивно любым адаптером, наполняется в release-пайплайне `.github/workflows/split.yml` (SPA панели + бандл тулбара). Это продовая поставка.
2. **Адаптерный `resources/dist/`** — fallback для монорепы после `make build-panel`.
3. **GitHub Pages CDN** — последний fallback.

| Адаптер | Источник | Раздаётся как |
|---------|----------|---------------|
| Symfony | `Resources/public/bundle.js` (после `make build-panel` или `app-dev-panel:assets:install`) → `/bundles/appdevpanel`; иначе `FrontendAssets::path()` через `AdpAssetsController` → `/_adp-assets` | `/bundles/appdevpanel` или `/_adp-assets` |
