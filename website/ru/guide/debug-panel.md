...
Рендеринг панели выполняется на уровне API (`PanelController`), который не зависит от фреймворка. Каждый адаптер просто маршрутизирует `/debug` и `/debug/*` на тот же `ApiApplication`, который обрабатывает API-запросы.
