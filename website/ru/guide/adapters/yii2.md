...
Поддерживает все коллекторы Kernel, а также специфичный для Yii 2 сбор данных: запросы к БД через `DbProfilingTarget` (логгер Yii), захват логов в реальном времени через `DebugLogTarget`, события почты и профилирование ассет-бандлов.
