...
| <class>AppDevPanel\Adapter\Yii3\Api\YiiApiMiddleware</class> | Перехватывает `/debug/*` (SPA панели + `/debug/api/*`) и `/inspect/api/*`, делегируя их ADP API. Должен стоять перед `Router`, чтобы маршруты приложения не затеняли ADP, и после `ErrorCatcher`, чтобы исключения корректно ловились. Сами файлы статики панели раздаются веб-сервером напрямую из публичной директории, куда адаптер кладёт симлинк (`@public/app-dev-panel`), а не через этот middleware. |
