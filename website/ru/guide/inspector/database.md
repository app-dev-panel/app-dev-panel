...
Просмотр таблиц базы данных, схемы и записей, выполнение SQL-запросов и анализ планов выполнения.

![Инспектор базы данных — Список таблиц](/images/inspector/database.png)

## Обозреватель таблиц

Список всех таблиц базы данных с количеством столбцов и записей. Нажмите **View**, чтобы увидеть схему и записи таблицы.
...
| Записи | Постраничные строки данных (по умолчанию 50, максимум 1000) |
...
Выполняйте любые SQL-запросы прямо из панели. Поддерживаются параметризованные запросы для безопасности.

## Планы EXPLAIN

Запускайте `EXPLAIN` или `EXPLAIN ANALYZE` для запросов, чтобы увидеть планы выполнения — полезно для отладки медленных запросов.
...
| GET | `/inspect/api/table/{name}?limit=50&offset=0` | Схема таблицы + постраничные записи |
...
::: warning
SQL-запросы выполняются на рабочей базе данных. Используйте с осторожностью в production-окружениях.
:::
