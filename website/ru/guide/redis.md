...
- **Команда**: имя, аргументы, возвращаемое значение
- **Время**: длительность выполнения в секундах
- **Ошибки**: сообщение об ошибке при сбое
- **Соединение**: какое Redis-соединение использовалось
- **Источник**: файл и строка вызова
...
Адаптеры фреймворков отвечают за перехват Redis-вызовов и передачу данных в коллектор.

## Интеграция с фреймворками

### Laravel

Самый простой вариант — Laravel предоставляет `Redis::listen()` из коробки:
...
ADP также предоставляет **live-инспектор Redis** на странице `/inspector/redis` с двумя вкладками:

**Keys** — просмотр, поиск, просмотр и удаление ключей:
- Поиск по паттерну (например, `user:*`)
- Отображение значений с учётом типа (string, list, set, zset, hash, stream)
- Информация о TTL
- Удаление отдельных ключей или очистка всей базы

**Server Info** — полная информация о Redis-сервере из команды `INFO`.

Инспектор требует `\Redis` (расширение phpredis), зарегистрированный в DI-контейнере.

## Панель отладки

Панель отладки показывает данные Redis с:
...
- **Список команд**: фильтруемый, с раскрываемыми деталями каждой команды
