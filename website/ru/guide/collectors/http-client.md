...
Коллектор получает данные от <class>\AppDevPanel\Kernel\Collector\HttpClientInterfaceProxy</class> — декоратора PSR-18 <class>Psr\Http\Client\ClientInterface</class>. Каждый вызов `$client->sendRequest($request)` автоматически перехватывается, замеряется по времени и записывается.
