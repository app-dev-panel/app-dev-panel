...
<class>\AppDevPanel\Kernel\Collector\RedisCollector</class> собирает все Redis-команды, выполненные во время запроса — SET, GET, DEL, INCR и любые другие команды — с таймингом, отслеживанием ошибок и поддержкой нескольких соединений.

## Что собирает

| Поле | Описание |
|------|----------|
| `connection` | Имя соединения Redis (например, `default`, `cache`) |
...
Коллектор не зависит от фреймворка. Он принимает нормализованные данные через `logCommand()`:
...
ADP предоставляет инспектор Redis в реальном времени по адресу `/inspector/redis`. Полное описание эндпоинтов см. в [Inspector API](/ru/api/inspector#redis).

**Вкладка Keys** — просмотр, поиск, отображение и удаление ключей Redis:
...
**Вкладка Server Info** — полная информация о сервере Redis из команды `INFO`.
...
Панель отладки отображает данные Redis, собранные во время запроса:

- **Карточки сводки** — общее количество команд, общее время, количество ошибок, соединения
...
- **Раскрываемые детали** — аргументы, результат, сообщение об ошибке, место вызова
