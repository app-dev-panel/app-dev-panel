...
| `traceCount` | Количество различных трейсов |
...
Коллектор получает спаны от адаптера OpenTelemetry `SpanExporter`. Когда ваше приложение использует OpenTelemetry SDK для трассировки, спаны экспортируются в этот коллектор вместо (или в дополнение к) внешних бэкендов, таких как Jaeger или Zipkin.

## Панель отладки

- **Представление трейсов** — спаны сгруппированы по ID трейса
- **Временная шкала спанов** — визуальная временная шкала длительности спанов
- **Подсветка ошибок** — спаны с ошибками отмечены красным
