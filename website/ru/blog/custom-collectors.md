...
description: "Пошаговое руководство по созданию пользовательских коллекторов ADP для сбора доменных данных в панели отладки."
---
...
Коллектор в ADP реализует интерфейс <class>AppDevPanel\Kernel\Collector\CollectorInterface</class>. Во время запроса (или консольной команды) коллектор накапливает данные. Когда отладчик завершает сброс в конце жизненного цикла, данные каждого коллектора сериализуются и записываются в хранилище. Затем API передаёт их фронтенду.
...
Создадим коллектор, отслеживающий взаимодействия с платёжным шлюзом. Создайте класс, реализующий <class>AppDevPanel\Kernel\Collector\CollectorInterface</class>:
...
Метод `getName()` возвращает уникальный идентификатор, используемый как ключ хранилища. Метод `collect()` возвращает payload данных для сериализации. Опциональный <class>AppDevPanel\Kernel\Collector\SummaryCollectorInterface</class> предоставляет сводные данные, отображаемые в тулбаре.
