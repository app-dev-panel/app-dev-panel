...
final class AppDevPanelServiceProvider extends ServiceProvider
{
    ...
    private function registerCollectors(): void
    {
        ...
        if ($collectors['request'] ?? true) {
            $this->app->singleton(RequestCollector::class, fn () => new RequestCollector(
                $this->app->make(TimelineCollector::class),
            ));
            $this->collectorClasses[] = RequestCollector::class;

            $this->app->singleton(WebAppInfoCollector::class, fn () => new WebAppInfoCollector(
                $this->app->make(TimelineCollector::class),
                'Laravel',
            ));
        ...
        if ($collectors['exception'] ?? true) {
            $this->app->singleton(ExceptionCollector::class, fn () => new ExceptionCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['log'] ?? true) {
            $this->app->singleton(LogCollector::class, fn () => new LogCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['event'] ?? true) {
            $this->app->singleton(EventCollector::class, fn () => new EventCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['service'] ?? true) {
            $this->app->singleton(ServiceCollector::class, fn () => new ServiceCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['http_client'] ?? true) {
            $this->app->singleton(HttpClientCollector::class, fn () => new HttpClientCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['var_dumper'] ?? true) {
            $this->app->singleton(VarDumperCollector::class, fn () => new VarDumperCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['command'] ?? true) {
            $this->app->singleton(CommandCollector::class, fn () => new CommandCollector(
                $this->app->make(TimelineCollector::class),
            ));
            $this->collectorClasses[] = CommandCollector::class;

            $this->app->singleton(ConsoleAppInfoCollector::class, fn () => new ConsoleAppInfoCollector(
                $this->app->make(TimelineCollector::class),
                'Laravel',
            ));
        ...
        if ($collectors['database'] ?? true) {
            $this->app->singleton(DatabaseCollector::class, fn () => new DatabaseCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['cache'] ?? true) {
            $this->app->singleton(CacheCollector::class, fn () => new CacheCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['mailer'] ?? true) {
            $this->app->singleton(MailerCollector::class, fn () => new MailerCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['queue'] ?? true) {
            $this->app->singleton(QueueCollector::class, fn () => new QueueCollector(
                $this->app->make(TimelineCollector::class),
            ));
        ...
        if ($collectors['router'] ?? true) {
            ...
            $this->app->singleton(RouterDataExtractor::class, fn () => new RouterDataExtractor(
                $this->app->make(RouterCollector::class),
    ...
    private function registerApiServices(): void
    {
        ...
        // Path resolver
        $this->app->singleton(PathResolverInterface::class, fn () => new PathResolver(
            base_path(),
            storage_path(),
        ));

        // JSON response factory
        $this->app->singleton(JsonResponseFactoryInterface::class, fn () => new JsonResponseFactory(
            $this->app->make(ResponseFactoryInterface::class),
            $this->app->make(StreamFactoryInterface::class),
        ));

        // Service registry
        $this->app->singleton(ServiceRegistryInterface::class, fn () => new FileServiceRegistry(
            $config->get('app-dev-panel.storage.path') . '/services',
        ));

        // Collector repository
        $this->app->singleton(CollectorRepositoryInterface::class, fn () => new CollectorRepository(
            $this->app->make(StorageInterface::class),
        ));

        // Middleware
        $this->app->singleton(IpFilterMiddleware::class, fn () => new IpFilterMiddleware(
            $this->app->make(ResponseFactoryInterface::class),
            $this->app->make(StreamFactoryInterface::class),
            $config->get('app-dev-panel.api.allowed_ips', ['127.0.0.1', '::1']),
        ));

        $this->app->singleton(TokenAuthMiddleware::class, fn () => new TokenAuthMiddleware(
            $this->app->make(ResponseFactoryInterface::class),
            $this->app->make(StreamFactoryInterface::class),
            $config->get('app-dev-panel.api.auth_token', ''),
        ));

        $this->app->singleton(ResponseDataWrapper::class, fn () => new ResponseDataWrapper(
            $this->app->make(JsonResponseFactoryInterface::class),
        ));

        $this->app->singleton(InspectorProxyMiddleware::class, fn () => new InspectorProxyMiddleware(
        ...
        // Controllers
        $this->app->singleton(DebugController::class, fn () => new DebugController(
            $this->app->make(JsonResponseFactoryInterface::class),
        ...
        $this->app->singleton(IngestionController::class, fn () => new IngestionController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->make(StorageInterface::class),
        ));

        $this->app->singleton(ServiceController::class, fn () => new ServiceController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->make(ServiceRegistryInterface::class),
        ));

        $this->app->singleton(FileController::class, fn () => new FileController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->make(PathResolverInterface::class),
        ));

        $this->app->singleton(GitRepositoryProvider::class, fn () => new GitRepositoryProvider(
            $this->app->make(PathResolverInterface::class),
        ));

        $this->app->singleton(GitController::class, fn () => new GitController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->make(GitRepositoryProvider::class),
        ));

        $this->app->singleton(ComposerController::class, fn () => new ComposerController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->make(PathResolverInterface::class),
        ));

        $this->app->singleton(OpcacheController::class, fn () => new OpcacheController(
            $this->app->make(JsonResponseFactoryInterface::class),
        ));
        ...
        $this->app->singleton(DatabaseController::class, fn () => new DatabaseController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->make(SchemaProviderInterface::class),
        ));

        // Laravel config provider for inspector
        $this->app->singleton(LaravelConfigProvider::class, fn () => new LaravelConfigProvider(
            $this->app,
        ));
        $this->app->alias(LaravelConfigProvider::class, 'config.adp');

        $this->app->singleton(InspectController::class, fn () => new InspectController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app,
            $this->app->make('config')->all(),
        ));

        $this->app->singleton(InspectorCacheController::class, fn () => new InspectorCacheController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app,
        ));

        $this->app->singleton(TranslationController::class, fn () => new TranslationController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->bound('log') ? $this->app->make('log') : null,
            $this->app,
        ));

        $this->app->singleton(CommandController::class, fn () => new CommandController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->make(PathResolverInterface::class),
            $this->app,
        ));
        ...
        $this->app->singleton(LaravelRouteCollectionAdapter::class, fn () => new LaravelRouteCollectionAdapter(
            $this->app->make('router'),
        ));

        $this->app->singleton(LaravelUrlMatcherAdapter::class, fn () => new LaravelUrlMatcherAdapter(
            $this->app->make('router'),
        ));

        $this->app->singleton(RoutingController::class, fn () => new RoutingController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->make(LaravelRouteCollectionAdapter::class),
            $this->app->make(LaravelUrlMatcherAdapter::class),
        ));

        $this->app->singleton(RequestController::class, fn () => new RequestController(
            $this->app->make(JsonResponseFactoryInterface::class),
            $this->app->make(CollectorRepositoryInterface::class),
        ));

        // ApiApplication
        $this->app->singleton(ApiApplication::class, fn () => new ApiApplication(
            $this->app,
            $this->app->make(ResponseFactoryInterface::class),
            $this->app->make(StreamFactoryInterface::class),
        ));
        ...
        // Bridge controller
        $this->app->singleton(AdpApiController::class, fn () => new AdpApiController(
            $this->app->make(ApiApplication::class),
        ));
    }
    ...
    private function registerCliCommands(): void
    {
        ...
        $this->app->when(DebugResetCommand::class)->needs(Debugger::class)->give(fn () => $this->app->make(Debugger::class));
    }
